cript that trains an agent that can play Atari's Breakout
"""
...
def create_CNN_model(number_actions, frames=4, input_shape=(84, 84)):
    ...
    layer_5 = layers.Dense(units=512, activation="relu")(layer_4)

    # output layer is fully-connected linear layer with
    #    single output for each valid action
    outputs = layers.Dense(units=number_actions, activation="linear")(layer_5)
...
def training():
    ...
    dqn.fit(env, nb_steps17500, log_interval=10000, visualize=False, verbose=2)
